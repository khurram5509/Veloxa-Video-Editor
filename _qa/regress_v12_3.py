...
check("APP_VERSION = '14.5.0'", _APP_VERSION == "14.5.0",
...
check("is_newer('14.5.0', '13.0')", _newer("14.5.0", "13.0"))
...
check("Help menu has 'Check for Updates' item",
      # V14.5.0: ellipsis switched from '...' to '…' (single codepoint)
...
check("installer.iss AppVersion = 14.5.0", '"14.5.0"' in iss_src)
check("installer.iss EXE name = V14.5.0.exe",
      "Veloxa-Video-Editor-V14.5.0.exe" in iss_src)
...
check("build.ps1 builds V14.5.0 EXE",
      "Veloxa-Video-Editor-V14.5.0" in ps1_src)

# V14.5.0 crash-fix: stale C++-object guard in _start_update_check.
...
# ===========================================================================
# 13. V14.5.0: System / Light / Dark theme switcher
# ===========================================================================
section("V14.5.0: theme switcher")
...
# V14.5.0: light theme redesign — depth + hierarchy.
...
check("Appearance submenu added to menu bar",
      ...
      # V14.5.0: label changed from 'System (follow Windows)' to
...
# V14.5.0: actually run each template's filter graph through FFmpeg
...
# ===========================================================================
# 15. V14.5.0: updater download runs on a QThread (no more GUI freeze)
# ===========================================================================
section("V14.5.0: updater download worker")
...
check("download_installer chunk size = 64 KB (V14.5.0 perf fix)",
...
# ===========================================================================
# 16. V14.5.0: preview overlay clears + installer no longer side-by-side
# ===========================================================================
section("V14.5.0: preview clear + installer overwrite")
...
# ===========================================================================
# 17. V14.5.0: Single instance + HiDPI + responsive hardening
# ===========================================================================
section("V14.5.0: single-instance, HiDPI, min size")
...
# ===========================================================================
# 18. V14.5.0: single-instance ACK handshake + dynamic version log
# ===========================================================================
section("V14.5.0: single-instance ACK + dynamic persistence version")
...
# ===========================================================================
# 19. V14.5.0: macOS source-level support + GitHub Actions
# ===========================================================================
section("V14.5.0: cross-platform foundation")
...
mac_assets = [
    {"name": "Veloxa-Video-Editor-V14.5.0-Setup.exe"},
    {"name": "Veloxa-Video-Editor-V14.5.0-macOS.dmg"},
]
...
# ===========================================================================
# V14.5.0 — Parallel CPU encoder slot + add-files-while-batch-runs
# ===========================================================================
section("V14.5.0 — CPU/GPU parallel slot + safety + queue-add")
...
# ===========================================================================
# V14.5.0 — first-video-stuck-at-0% progress fix
# ===========================================================================
section("V14.5.0 — progress emission fix")
...
# ===========================================================================
# V14.5.0 — audio-template preview pane fix
# ===========================================================================
section("V14.5.0 — audio-template preview pane")
...
# ===========================================================================
# V14.5.0 — platform-asset routing must never mix .exe and .dmg
# ===========================================================================
section("V14.5.0 — Win → .exe only / Mac → .dmg only (no mixing)")
...
# ===========================================================================
# V14.5.0 — auto-assign audio visuals at add-to-queue time
# ===========================================================================
section("V14.5.0 — auto-assign audio visuals on add")
...
# ===========================================================================
# V14.5.0 — light-theme queue-row text was invisible (white-on-white)
# ===========================================================================
section("V14.5.0 — light theme: queue-row labels must be readable")
...
# ===========================================================================
# V14.5.0 — build mode: --onedir (was --onefile, caused python314.dll
# LoadLibrary failures after Windows in-app updates)
# ===========================================================================
section("V14.5.0 — Windows build uses --onedir (no _MEI extract race)")
...
# ===========================================================================
# V14.5.0 — settings tabs wrap in QScrollArea (macOS row-overlap fix)
# ===========================================================================
section("V14.5.0 — settings tabs scroll instead of overlapping on macOS")
...
# ===========================================================================
# V14.5.0 — audio row label respects active audio template
# ===========================================================================
section("V14.5.0 — audio row label: template-active stops showing "
        "'(visual needed)'")
...
# ===========================================================================
# V14.5.0 — macOS menubar fix: every top-level entry is a real menu
# ===========================================================================
section("V14.5.0 — macOS menubar: Tools / Help / Appearance")

# macOS native QMenuBar silently DROPS top-level QActions added via
# ``mb.addAction(...)``. Only proper submenus (``mb.addMenu(...)``) show
# up. In V14.5.0 and earlier this left macOS users with ONLY the
...
# ===========================================================================
# V14.5.0 — GPU detection is per-machine, cache key includes machine ID,
#           Tools menu can force a re-detect, status bar shows the GPU
# ===========================================================================
section("V14.5.0 — GPU is auto-detected per physical PC, never baked")
...
check("encoders schema bumped to 3 (cache invalidated by V14.5.0)",
...
# ===========================================================================
# V14.5.0 — resume interrupted batches + opt-in crash reporter
# ===========================================================================
section("V14.5.0 — resume + crash reporter")
